...
class Test_Edit_XO:
    ...
    @allure.title("进入XO编辑页面")
    def test_XO_edit(self, driver):
        xo = LeadlisteningPage(driver)
        xo.printout_booking()
        xo.riseXO_edit()

    @pytest.mark.parametrize("caseinfo2",
                             read_testcase("test_case", "\datas\invoice_edit\Invoice_edit_hotel.yaml"))
    @allure.title("編輯酒店項目")
    def test_XO_edit_hotel(self, driver, caseinfo2):
        xo = QuotationEditPage(driver)
        xo.quotation_edit_hotel(caseinfo2["texthotelfare_invoice"], caseinfo2["texthotelfare_xo"], caseinfo2["texthotelfare_num"],
                                caseinfo2["texthotelfare_remark"])
